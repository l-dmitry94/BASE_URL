...
`,{onShow:r=>{window.addEventListener("keydown",s),e=r.element().querySelector(".footer__modal-close"),e.addEventListener("click",()=>r.close()),document.body.classList.add("modal-open")},onClose:()=>{e.removeEventListener("click",()=>t.close()),window.removeEventListener("keydown",s),document.body.classList.remove("modal-open")}});t.show();function s(r){r.code==="Escape"&&t.close()}},c={productsFilters:document.querySelector(".products__filters-input"),productsFiltersSelect:document.querySelector(".products__filters-select"),productsCards:document.querySelector(".product__cards"),cardWrapper:document.querySelector(".cart__wrapper")};let h={keyword:null,category:null,page:1,limit:6},v=JSON.stringify(h);const y=document.querySelector(".footer__form");y.addEventListener("submit",b);function b(e){e.preventDefault();const{value:t}=e.currentTarget.elements.email;S(t),e.target.reset()}async function S(e){try{(await m({method:"POST",data:{email:e}})).statusText==="Created"&&_()}catch({response:t}){t.statusText==="Conflict"&&g()}}function p(e){return e.map(({_id:t,name:s,img:r,category:o,price:a,size:l,popularity:f})=>`<div class="product__cards">
            <ul class="cards">
